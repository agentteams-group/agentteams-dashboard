...
if echo "${dashboard_version_line}" | grep -q 'v1\.2\.4\.9'; then
    pass "Dashboard has independent default version (v1.2.4.9)"
else
...
if echo "${_exec_result}" | grep -q "EXTRACTION_FAILED\|FUNCTION_NOT_FOUND"; then
    ...
    if echo "${_exec_result}" | grep -q "RESULT_VERSION=v1.2.4.9"; then
        pass "Exec non-interactive: default version = v1.2.4.9"
    else
    ...
    if echo "${_exec_result}" | grep -q "RESULT_IMAGE=.*agentteams-dashboard:v1.2.4.9"; then
...
if echo "${_qs_result}" | grep -q "EXTRACTION_FAILED\|FUNCTION_NOT_FOUND"; then
    ...
    if echo "${_qs_result}" | grep -q "RESULT_IMAGE=.*agentteams-dashboard:v1.2.4.9"; then
