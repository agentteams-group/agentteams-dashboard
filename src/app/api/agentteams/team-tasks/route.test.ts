...
describe('normalizeStatus', () => {
  it.each([
    ...
    ['PENDING', 'pending'],
    ['queued', 'pending'],
    // AgentTeams TaskMeta raw statuses (controller normalizeTaskStatus):
    // planned→pending, delegated→assigned, submitted→in_progress,
    // revision keeps its own state, cancelled→blocked.
    ['planned', 'pending'],
    ['assigned', 'assigned'],
    ['delegated', 'assigned'],
    ['todo', 'assigned'],
