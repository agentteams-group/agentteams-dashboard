  ...
  const sectionsWithNotifications = useMemo(() => {
    ...
    notifications.forEach((n) => {
      if (!n.read && n.timestamp > fiveMinutesAgo) {
        ...
        if (msg.includes('infra') || msg.includes('基础设施')) { /* ops section removed; legacy fallback */ }
        if (msg.includes('k8s') || msg.includes('kubernetes')) { /* ops section removed; legacy fallback */ }
      }
    });
    return sectionSet;
  }, [notifications]);
