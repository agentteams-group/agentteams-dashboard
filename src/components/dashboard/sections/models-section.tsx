...
function RouteDialog({ open, route, providerNames, fallbackConfigWritable, onOpenChange }: { open: boolean; route: AiRoute | null; providerNames: string[]; fallbackConfigWritable: boolean; onOpenChange: (_open: boolean) => void }) {
  ...
  return <Dialog open={open} onOpenChange={onOpenChange}><DialogContent className="max-h-[90vh] max-w-4xl overflow-y-auto"><DialogHeader><DialogTitle>{editing ? `编辑路由 - ${route?.name}` : '创建 AI 路由'}</DialogTitle><DialogDescription>路由决定请求模型别名如何匹配到上游提供商。每个上游可配置独立的模型映射。</DialogDescription></DialogHeader><div className="space-y-4 py-2">
    ...
    <div className="space-y-3 rounded-md border p-3"><p className="text-xs text-muted-foreground">新增路由不会自动授权已有 Worker。保存后请在 Consumers 中检查绑定范围，并在 Worker 编辑页验证已保存模型；提供商连接成功不代表 Worker 已有访问权限。</p><div className="flex items-center gap-2"><Input id="route-auth" className="size-4" type="checkbox" checked={form.authConfig.enabled} disabled={pending} onChange={(event) => setForm({ ...form, authConfig: { ...form.authConfig, enabled: event.target.checked } })} /><Label htmlFor="route-auth">启用认证</Label></div>{form.authConfig.enabled && <div className="flex items-center gap-2"><Input id="key-auth" className="size-4" type="checkbox" checked={form.authConfig.allowedCredentialTypes.includes('key-auth')} disabled={pending} onChange={(event) => setForm({ ...form, authConfig: { ...form.authConfig, allowedCredentialTypes: event.target.checked ? ['key-auth'] : [] } })} /><Label htmlFor="key-auth">Key Auth</Label></div>}
      {form.authConfig.enabled && <div className="space-y-2"><Label htmlFor="route-consumers">授权 Consumer（逗号分隔）</Label><ConsumerNamesInput value={form.authConfig.allowedConsumers || []} disabled={pending} onChange={(allowedConsumers) => setForm({ ...form, authConfig: { ...form.authConfig, allowedConsumers } })} /><p className="text-xs text-muted-foreground">只授权此路由；Worker Consumer 通常为 worker-加运行时名称。保存后在 Worker 编辑页验证，不要通过关闭认证解决授权失败。</p></div>}
    </div>
...
function ConsumerNamesInput({ value, disabled, onChange }: { value: string[]; disabled: boolean; onChange: (_names: string[]) => void }) {
  const [text, setText] = useState(value.join(', '));
  return <Input id="route-consumers" value={text} disabled={disabled} placeholder="worker-alice, manager" onChange={(event) => {
    setText(event.target.value);
    onChange([...new Set(event.target.value.split(',').map((name) => name.trim()).filter(Boolean))]);
  }} />;
}
